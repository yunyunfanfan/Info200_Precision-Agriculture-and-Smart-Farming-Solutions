...
def add_box(ax, x, y, w, h, text, fc="#EAF4F4", ec="#264653", fontsize=10):
    rect = Rectangle((x, y), w, h, facecolor=fc, edgecolor=ec, linewidth=1.8)
    ax.add_patch(rect)
    ax.text(x + w / 2, y + h / 2, text, ha="center", va="center", fontsize=fontsize)


def add_arrow(ax, start, end, color="#555555", text=None, text_pos=None):
...
def draw_architecture():
    ...
    add_box(ax, 0.5, 3.8, 2.0, 1.1, "Field Sensors\nSoil moisture\nTemperature\npH", fc="#D8F3DC")
    add_box(ax, 0.5, 2.1, 2.0, 1.1, "Drone Imaging\nCrop health\nPest detection", fc="#D8F3DC")
    add_box(ax, 0.5, 0.4, 2.0, 1.1, "External Data\nWeather API\nMarket price feed", fc="#D8F3DC")

    add_box(ax, 3.3, 2.2, 2.2, 1.6, "IoT Gateway\nData validation\nEdge buffering", fc="#CDE7F0")
    add_box(ax, 6.1, 2.2, 2.2, 1.6, "Smart Farming Platform\nAnalytics engine\nDecision rules\nAlerts", fc="#FCECC9")
    add_box(ax, 9.0, 3.5, 2.2, 1.0, "Farmer App\nTasks & alerts", fc="#F1D5DA")
    add_box(ax, 9.0, 2.0, 2.2, 1.0, "Cooperative Dashboard\nYield, water, logistics", fc="#F1D5DA")
    add_box(ax, 9.0, 0.5, 2.2, 1.0, "Manager Portal\nReports & planning", fc="#F1D5DA")

    add_arrow(ax, (2.5, 4.35), (3.3, 3.4))
    add_arrow(ax, (2.5, 2.65), (3.3, 2.95))
    add_arrow(ax, (2.5, 0.95), (3.3, 2.5))
    add_arrow(ax, (5.5, 3.0), (6.1, 3.0), text="clean data", text_pos=(5.8, 3.25))
    add_arrow(ax, (8.3, 3.3), (9.0, 4.0))
    add_arrow(ax, (8.3, 3.0), (9.0, 2.5))
    add_arrow(ax, (8.3, 2.7), (9.0, 1.0))
...
def draw_dfd():
    ...
    add_box(ax, 0.4, 4.9, 2.0, 1.0, "Farmers", fc="#E8F1F2")
    add_box(ax, 0.4, 2.8, 2.0, 1.0, "Agronomists", fc="#E8F1F2")
    add_box(ax, 0.4, 0.7, 2.0, 1.0, "Logistics Providers", fc="#E8F1F2")

    add_box(ax, 3.0, 4.5, 2.5, 1.3, "1.0 Collect Farm Data", fc="#D9ED92")
    add_box(ax, 3.0, 2.5, 2.5, 1.3, "2.0 Analyse Conditions", fc="#D9ED92")
    add_box(ax, 3.0, 0.5, 2.5, 1.3, "3.0 Coordinate Operations", fc="#D9ED92")

    add_box(ax, 6.4, 4.7, 2.2, 1.0, "D1 Sensor Data Store", fc="#FFF3B0")
    add_box(ax, 6.4, 2.7, 2.2, 1.0, "D2 Farm Records", fc="#FFF3B0")
    add_box(ax, 6.4, 0.7, 2.2, 1.0, "D3 Logistics Schedule", fc="#FFF3B0")

    add_box(ax, 9.4, 2.3, 2.1, 1.6, "Decision Dashboard\nAlerts\nRecommendations\nReports", fc="#F7D6E0")

    add_arrow(ax, (2.4, 5.4), (3.0, 5.15), text="field inputs", text_pos=(2.7, 5.65))
    add_arrow(ax, (2.4, 3.3), (3.0, 3.15), text="expert advice", text_pos=(2.7, 3.55))
    add_arrow(ax, (2.4, 1.2), (3.0, 1.15), text="delivery status", text_pos=(2.75, 1.45))

    add_arrow(ax, (5.5, 5.15), (6.4, 5.2))
    add_arrow(ax, (5.5, 3.15), (6.4, 3.2))
    add_arrow(ax, (5.5, 1.15), (6.4, 1.2))

    add_arrow(ax, (8.6, 5.2), (4.25, 3.8), color="#6C757D")
    add_arrow(ax, (8.6, 3.2), (4.25, 3.8), color="#6C757D")
    add_arrow(ax, (8.6, 1.2), (4.25, 1.8), color="#6C757D")

    add_arrow(ax, (5.5, 3.15), (9.4, 3.1), text="insights", text_pos=(7.5, 3.45))
    add_arrow(ax, (11.5, 3.6), (2.4, 5.4), color="#BC4749")
    add_arrow(ax, (11.5, 3.1), (2.4, 3.3), color="#BC4749")
    add_arrow(ax, (11.5, 2.6), (2.4, 1.2), color="#BC4749")
...
def draw_dashboard_mockup():
    ...
    add_box(ax, 0.2, 6.2, 11.4, 0.6, "Unified Dashboard: Precision Agriculture Monitoring", fc="#264653", ec="#264653", fontsize=13)
    ax.text(6.0, 6.5, "Unified Dashboard: Precision Agriculture Monitoring", color="white", ha="center", va="center", fontsize=13)
